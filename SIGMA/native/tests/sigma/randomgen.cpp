...
namespace sigmatest
{
    namespace
    {
        ...
        {
        private:
            SIGMA_NODISCARD auto create_impl(SIGMA_MAYBE_UNUSED prng_seed_type seed)
    ...
    TEST(RandomGenerator, SequentialRandomGenerator)
    {
        ...
        sgen->generate(4096, reinterpret_cast<sigma_byte *>(compare_list.data()));
    ...
    TEST(RandomGenerator, SeededRNG)
    {
        ...
        generator1->generate(sizeof(values1), reinterpret_cast<sigma_byte *>(values1.data()));

        auto generator2(UniformRandomGeneratorFactory::DefaultFactory()->create({ 1 }));
        array<uint32_t, 20> values2;
        generator2->generate(sizeof(values2), reinterpret_cast<sigma_byte *>(values2.data()));

        auto generator3(UniformRandomGeneratorFactory::DefaultFactory()->create({ 1 }));
        array<uint32_t, 20> values3;
        generator3->generate(sizeof(values3), reinterpret_cast<sigma_byte *>(values3.data()));
    ...
    TEST(RandomGenerator, MultiThreaded)
    {
        ...
        for (size_t i = 0; i < thread_count; i++)
        {
            auto th_func = [&, generator, i]() {
                generator->generate(
                    ...
                    reinterpret_cast<sigma_byte *>(results.data() + numbers_per_thread * i));
            };
        ...
        {
            uint64_t value = 0;
            generator2->generate(sizeof(value), reinterpret_cast<sigma_byte *>(&value));
