...
namespace sigmatest
{
    namespace util
    {
        ...
        TEST(IteratorTest, Iterate)
        {
            ...
            calls = 0;
            sum = 0;
            SIGMA_ITERATE(iter(0), 10, [&](auto I) {
                sum += I;
                calls++;
            });
            ASSERT_EQ(10, calls);
            ...
            sum = 0;
            SIGMA_ITERATE(iter(0, reverse_iter(0)), 10, [&](auto I) { sum += get<0>(I) + get<1>(I); });
        }
