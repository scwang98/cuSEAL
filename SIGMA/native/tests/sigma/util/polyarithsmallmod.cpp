...
namespace sigmatest
{
    namespace util
    {
        TEST(PolyArithSmallMod, ModuloPolyCoeffs)
        {
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly, 3, pool);
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly, 3, 2, pool);
                auto modulus(allocate_uint(2, pool));
                poly[0][0] = 2;
                poly[0][1] = 15;
                poly[0][2] = 77;
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly, 2, 3, 2, pool);
                auto modulus(allocate_uint(2, pool));
                poly[0][0][0] = 2;
                poly[0][0][1] = 15;
                poly[0][0][2] = 77;
        ...
        TEST(PolyArithSmallMod, AddPolyCoeffMod)
        {
            MemoryPool &pool = *global_variables::global_memory_pool;
            {
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly1, 3, pool);
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly2, 3, pool);
                poly1[0] = 1;
                poly1[1] = 3;
                poly1[2] = 4;
                poly2[0] = 1;
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly1, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly2, 3, 2, pool);
                poly1[0][0] = 1;
                poly1[0][1] = 3;
                poly1[0][2] = 4;
                poly1[1][0] = 0;
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly1, 2, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly2, 2, 3, 2, pool);
                poly1[0][0][0] = 1;
                poly1[0][0][1] = 3;
                poly1[0][0][2] = 4;
                poly1[0][1][0] = 0;
        ...
        TEST(PolyArithSmallMod, MultiplyPolyMonoCoeffMod)
        {
            MemoryPool &pool = *global_variables::global_memory_pool;
            {
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly, 4, pool);
                poly[0] = 1;
                poly[1] = 3;
                poly[2] = 4;
                poly[3] = 2;
                uint64_t mono_coeff = 3;
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(result, 4, pool);
                Modulus mod(5);
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly, 4, 2, pool);
                poly[0][0] = 1;
                poly[0][1] = 3;
                poly[0][2] = 4;
                poly[0][3] = 2;
                ...
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(result, 4, 2, pool);
            ...
            {
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly, 2, 4, 2, pool);
                poly[0][0][0] = 1;
                poly[0][0][1] = 3;
                poly[0][0][2] = 4;
                poly[0][0][3] = 2;
                ...
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(result, 2, 4, 2, pool);
                vector<Modulus> mod{ 5, 7 };
        ...
        TEST(PolyArithSmallMod, DyadicProductCoeffMod)
        {
            MemoryPool &pool = *global_variables::global_memory_pool;
            {
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly1, 3, pool);
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(poly2, 3, pool);
                SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(result, 3, pool);
                Modulus mod(13);
            ...
            }
            {
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly1, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(poly2, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(result, 3, 2, pool);
            ...
            }
            {
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly1, 2, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(poly2, 2, 3, 2, pool);
                SIGMA_ALLOCATE_ZERO_GET_POLY_ITER(result, 2, 3, 2, pool);
                vector<Modulus> mod{ 13, 7 };
