...
#include "sigma/util/mempool.h"
#include "sigma/util/ntt.h"
#include "sigma/util/pointer.h"
...
namespace sigmatest
{
    namespace util
    {
        ...
        {
            {
                ...
                Pointer<sigma_byte> pointer4 = pool.get_for_byte_count(1);
                Pointer<sigma_byte> pointer5 = pool.get_for_byte_count(2);
                Pointer<sigma_byte> pointer6 = pool.get_for_byte_count(1);
                pointer4.release();
                pointer5.release();
                pointer6.release();
            ...
            }
            {
                ...
                Pointer<sigma_byte> pointer = pool.get_for_byte_count(0);
                ASSERT_FALSE(pointer.is_set());
                pointer.release();
                ASSERT_TRUE(0LL == pool.pool_count());

                pointer = pool.get_for_byte_count(2);
                sigma_byte *allocation1 = pointer.get();
                ASSERT_TRUE(pointer.is_set());
                pointer.release();
                ASSERT_FALSE(pointer.is_set());
                ASSERT_TRUE(1LL == pool.pool_count());
                ...
                pointer = pool.get_for_byte_count(2);
                ASSERT_TRUE(allocation1 == pointer.get());
                Pointer<sigma_byte> pointer2 = pool.get_for_byte_count(2);
                sigma_byte *allocation2 = pointer2.get();
                ASSERT_FALSE(allocation2 == pointer.get());
                ASSERT_TRUE(pointer.is_set());
                pointer.release();
                pointer2.release();
                ...
                ASSERT_TRUE(allocation1 == pointer2.get());
                Pointer<sigma_byte> pointer3 = pool.get_for_byte_count(1);
                pointer.release();
                pointer2.release();
                pointer3.release();
                ASSERT_TRUE(2LL == pool.pool_count());
        ...
        TEST(MemoryPoolTests, PointerTestsMT)
        {
            ...
            {
                Pointer<sigma_byte> p1;
                ASSERT_FALSE(p1.is_set());
                ASSERT_TRUE(p1.get() == nullptr);

                p1 = pool.get_for_byte_count(bytes_per_uint64 * 1);
                sigma_byte *allocation1 = p1.get();
                ASSERT_TRUE(p1.is_set());
                ASSERT_TRUE(p1.get() != nullptr);
                ...
                Pointer<sigma_byte> p2;
                p2.acquire(p1);
                ASSERT_FALSE(p1.is_set());
                ASSERT_TRUE(p2.is_set());
                ASSERT_TRUE(p2.get() == allocation1);

                ConstPointer<sigma_byte> cp2;
                cp2.acquire(p2);
                ASSERT_FALSE(p2.is_set());
                ASSERT_TRUE(cp2.is_set());
                ASSERT_TRUE(cp2.get() == allocation1);
                ...
                Pointer<sigma_byte> p3 = pool.get_for_byte_count(bytes_per_uint64 * 1);
                ASSERT_TRUE(p3.is_set());
                ASSERT_TRUE(p3.get() == allocation1);

                Pointer<sigma_byte> p4 = pool.get_for_byte_count(bytes_per_uint64 * 2);
                ASSERT_TRUE(p4.is_set());
                sigma_byte *allocation2 = p4.get();
                swap(p3, p4);
                ASSERT_TRUE(p3.is_set());
                ASSERT_TRUE(p3.get() == allocation2);
                ASSERT_TRUE(p4.is_set());
        ...
        {
            MemoryPool &pool = *global_variables::global_memory_pool;
            vector<sigma_byte> bytes{ sigma_byte(0), sigma_byte(1), sigma_byte(2), sigma_byte(3), sigma_byte(4) };
