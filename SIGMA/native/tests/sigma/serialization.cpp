...
namespace sigmatest
{
    namespace
    {
        struct test_struct
        {
            ...
            streamoff save_size(compr_mode_type compr_mode) const
            {
                ...
                return static_cast<streamoff>(sizeof(Serialization::SIGMAHeader) + members_size);
            }
        };
    } // namespace
    ...
    {
        ASSERT_EQ(sizeof(Serialization::SIGMAHeader), Serialization::sigma_header_size);
        ...
        Serialization::SIGMAHeader invalid_header;
        invalid_header.magic = 0x1212;
        ASSERT_FALSE(Serialization::IsValidHeader(invalid_header));
        invalid_header.magic = Serialization::sigma_magic;
        ASSERT_EQ(invalid_header.header_size, Serialization::sigma_header_size);
        invalid_header.version_major = 0x02;
        ASSERT_FALSE(Serialization::IsValidHeader(invalid_header));
        invalid_header.version_major = SIGMA_VERSION_MAJOR;
        invalid_header.compr_mode = (compr_mode_type)0x03;
        ASSERT_FALSE(Serialization::IsValidHeader(invalid_header));
    }

    TEST(SerializationTest, SIGMAHeaderSaveLoad)
    {
        {
            // Serialize to stream
            Serialization::SIGMAHeader header, loaded_header;
            ...
            ASSERT_EQ(Serialization::sigma_magic, loaded_header.magic);
            ASSERT_EQ(Serialization::sigma_header_size, loaded_header.header_size);
            ASSERT_EQ(SIGMA_VERSION_MAJOR, loaded_header.version_major);
            ASSERT_EQ(SIGMA_VERSION_MINOR, loaded_header.version_minor);
    ...
    /*
        TEST(SerializationTest, SIGMAHeaderUpgrade)
        {
            legacy_headers::SIGMAHeader_3_4 header_3_4;
...

            {
                ...
                Serialization::LoadHeader(
                    reinterpret_cast<const sigma_byte *>(&header_3_4), sizeof(legacy_headers::SIGMAHeader_3_4), header);
                ASSERT_TRUE(Serialization::IsValidHeader(header));
    ...
    */
    ...
    TEST(SerializationTest, SaveLoadToBuffer)
    {
        ...
        {
            ASSERT_TRUE(sigma_byte{} == buffer[i]);
        }
