...
namespace sigmabench
{
    ...
    */
    ...
    public:
        ...
        BMEnv(const sigma::EncryptionParameters &parms)
            : parms_(parms), context_(parms_, true, sigma::sec_level_type::none)
        {
            keygen_ = std::make_shared<sigma::KeyGenerator>(context_);
            ...
            encryptor_ = std::make_shared<sigma::Encryptor>(context_, pk_, sk_);
            decryptor_ = std::make_shared<sigma::Decryptor>(context_, sk_);
            if (parms_.scheme() == sigma::scheme_type::bfv || parms_.scheme() == sigma::scheme_type::bgv)
            {
                batch_encoder_ = std::make_shared<sigma::BatchEncoder>(context_);
            }
            else if (parms_.scheme() == sigma::scheme_type::ckks)
            {
                ckks_encoder_ = std::make_shared<sigma::CKKSEncoder>(context_);
            }
            evaluator_ = std::make_shared<sigma::Evaluator>(context_);
        ...
        Getter methods.
        */
        SIGMA_NODISCARD const sigma::EncryptionParameters &parms() const
        {
            return parms_;
        }

        SIGMA_NODISCARD const sigma::SIGMAContext &context() const
        {
            return context_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::KeyGenerator> keygen()
        {
            return keygen_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::Encryptor> encryptor()
        {
            return encryptor_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::Decryptor> decryptor()
        {
            return decryptor_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::BatchEncoder> batch_encoder()
        {
            return batch_encoder_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::CKKSEncoder> ckks_encoder()
        {
            return ckks_encoder_;
        }

        SIGMA_NODISCARD std::shared_ptr<sigma::Evaluator> evaluator()
        {
            return evaluator_;
        }

        SIGMA_NODISCARD sigma::SecretKey &sk()
        {
            return sk_;
        }

        SIGMA_NODISCARD const sigma::SecretKey &sk() const
        {
            return sk_;
        }

        SIGMA_NODISCARD sigma::PublicKey &pk()
        {
            return pk_;
        }

        SIGMA_NODISCARD const sigma::PublicKey &pk() const
        {
            return pk_;
        }

        SIGMA_NODISCARD sigma::RelinKeys &rlk()
        {
            return rlk_;
        }

        SIGMA_NODISCARD const sigma::RelinKeys &rlk() const
        {
            return rlk_;
        }

        SIGMA_NODISCARD sigma::GaloisKeys &glk()
        {
            return glk_;
        }

        SIGMA_NODISCARD const sigma::GaloisKeys &glk() const
        {
            return glk_;
        }

        SIGMA_NODISCARD const std::vector<std::uint32_t> &galois_elts_all() const
        {
            return galois_elts_all_;
        }

        SIGMA_NODISCARD std::vector<std::uint64_t> &msg_uint64()
        {
            return msg_uint64_;
        }

        SIGMA_NODISCARD std::vector<double> &msg_double()
        {
            return msg_double_;
        }

        SIGMA_NODISCARD std::vector<sigma::Plaintext> &pt()
        {
            return pt_;
        }

        SIGMA_NODISCARD std::vector<sigma::Ciphertext> &ct()
        {
            return ct_;
        }
        ...
        */
        SIGMA_NODISCARD double safe_scale()
        {
        ...
        */
        void randomize_array_mod(std::uint64_t *data, std::size_t count, const sigma::Modulus &modulus)
        {
            // For the purpose of benchmark, avoid using sigma::UniformRandomGenerator, as it degrades
            ...
            // This method is not used for random number generation in Microsoft SIGMA.
        ...
        */
        void randomize_poly_rns(std::uint64_t *data, const sigma::EncryptionParameters &parms)
        {
            std::size_t coeff_count = parms.poly_modulus_degree();
            std::vector<sigma::Modulus> coeff_modulus = parms.coeff_modulus();
        ...
        */
        void randomize_ct_bfv(sigma::Ciphertext &ct)
        {
        ...
        */
        void randomize_ct_bgv(sigma::Ciphertext &ct)
        {
            if (ct.parms_id() != context_.first_parms_id())
            {
                ct.resize(context_, std::size_t(2));
        ...
        */
        void randomize_ct_ckks(sigma::Ciphertext &ct)
        {
            if (ct.parms_id() != context_.first_parms_id())
            {
                ct.resize(context_, std::size_t(2));
        ...
        */
        void randomize_pt_bfv(sigma::Plaintext &pt)
        {
            pt.resize(parms_.poly_modulus_degree());
            pt.parms_id() = sigma::parms_id_zero;
        ...
        */
        void randomize_pt_bgv(sigma::Plaintext &pt)
        {
            randomize_pt_bfv(pt);
        }
        ...
        */
        void randomize_pt_ckks(sigma::Plaintext &pt)
        {
            auto &parms = context_.first_context_data()->parms();
    ...
    private:
        sigma::EncryptionParameters parms_;
        sigma::SIGMAContext context_;
        std::shared_ptr<sigma::KeyGenerator> keygen_{ nullptr };
        std::shared_ptr<sigma::Encryptor> encryptor_{ nullptr };
        std::shared_ptr<sigma::Decryptor> decryptor_{ nullptr };
        std::shared_ptr<sigma::BatchEncoder> batch_encoder_{ nullptr };
        std::shared_ptr<sigma::CKKSEncoder> ckks_encoder_{ nullptr };
        std::shared_ptr<sigma::Evaluator> evaluator_{ nullptr };
