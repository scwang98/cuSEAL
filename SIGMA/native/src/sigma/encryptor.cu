...
//        auto noise = DeviceArray<uint64_t>(coeff_count * coeff_modulus_size);
        ...
        kernel_util::sample_poly_cbd(
                random_generator_, device_coeff_modulus.get(), coeff_modulus_size, coeff_count, destination.temp_noise_.get(), stream);
        ...
        for (size_t i = 0; i < coeff_modulus_size; i++) {
            kernel_util::dyadic_product_coeffmod(
                    ...
                    coeff_count, 1, 1, coeff_modulus[i], c0 + i * coeff_count, stream);

            // Transform the noise e into NTT representation
            kernel_util::g_ntt_negacyclic_harvey(destination.temp_noise_.get() + i * coeff_count, coeff_count, ntt_tables[i], stream);
            ...
            kernel_util::add_negate_add_poly_coeffmod(
                    ...
                    coeff_count, coeff_modulus[i].value(), c0 + i * coeff_count, stream);
        }
