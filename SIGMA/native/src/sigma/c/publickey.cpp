...
SIGMA_C_FUNC PublicKey_UnsafeLoad(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    try
    {
        *in_bytes = util::safe_cast<int64_t>(
            pkey->unsafe_load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
...
SIGMA_C_FUNC PublicKey_Load(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    {
        *in_bytes = util::safe_cast<int64_t>(
            pkey->load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
