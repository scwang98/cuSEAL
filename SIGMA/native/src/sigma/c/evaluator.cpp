...
// Enables access to private members of sigma::Evaluator.
using ph = struct sigma::Evaluator::EvaluatorPrivateHelper
{
...
SIGMA_C_FUNC Evaluator_Relinearize(void *thisptr, void *encrypted, void *relin_keys, void *destination, void *pool)
{
    Evaluator *eval = FromVoid<Evaluator>(thisptr);
    IfNullRet(eval, E_POINTER);
    Ciphertext *encrypted_ptr = FromVoid<Ciphertext>(encrypted);
...
SIGMA_C_FUNC Evaluator_TransformToNTT1(void *thisptr, void *plain, uint64_t *parms_id, void *destination_ntt, void *pool)
{
    Evaluator *eval = FromVoid<Evaluator>(thisptr);
    IfNullRet(eval, E_POINTER);
    Plaintext *plain_ptr = FromVoid<Plaintext>(plain);
...
SIGMA_C_FUNC Evaluator_ComplexConjugate(void *thisptr, void *encrypted, void *galois_keys, void *destination, void *pool)
{
    Evaluator *eval = FromVoid<Evaluator>(thisptr);
    IfNullRet(eval, E_POINTER);
    Ciphertext *encrypted_ptr = FromVoid<Ciphertext>(encrypted);
