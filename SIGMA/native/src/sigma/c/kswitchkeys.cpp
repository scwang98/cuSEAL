...
// Enables access to private members of sigma::PublicKey.
...
SIGMA_C_FUNC KSwitchKeys_RawSize(void *thisptr, uint64_t *size)
{
    KSwitchKeys *keys = FromVoid<KSwitchKeys>(thisptr);
    IfNullRet(keys, E_POINTER);
    IfNullRet(size, E_POINTER);
...
SIGMA_C_FUNC KSwitchKeys_UnsafeLoad(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    try
    {
        *in_bytes = util::safe_cast<int64_t>(
            keys->unsafe_load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
...
SIGMA_C_FUNC KSwitchKeys_Load(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    {
        *in_bytes = util::safe_cast<int64_t>(
            keys->load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
