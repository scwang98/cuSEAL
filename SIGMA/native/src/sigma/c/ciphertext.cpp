...
// Enables access to private members of sigma::Ciphertext.
using ph = struct sigma::Ciphertext::CiphertextPrivateHelper
{
...
SIGMA_C_FUNC Ciphertext_Create1(void *memoryPoolHandle, void **ciphertext)
{
...
SIGMA_C_FUNC Ciphertext_Create2(void *copy, void **ciphertext)
{
...
SIGMA_C_FUNC Ciphertext_Create3(void *context, void *pool, void **ciphertext)
{
...
SIGMA_C_FUNC Ciphertext_Create4(void *context, uint64_t *parms_id, void *pool, void **ciphertext)
{
    const SIGMAContext *ctx = FromVoid<SIGMAContext>(context);
    IfNullRet(ctx, E_POINTER);
...
SIGMA_C_FUNC Ciphertext_UnsafeLoad(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    try
    {
        *in_bytes = util::safe_cast<int64_t>(
            cipher->unsafe_load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
...
SIGMA_C_FUNC Ciphertext_Load(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    {
        *in_bytes = util::safe_cast<int64_t>(
            cipher->load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
