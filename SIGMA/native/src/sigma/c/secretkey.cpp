...
SIGMA_C_FUNC SecretKey_UnsafeLoad(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    try
    {
        *in_bytes = util::safe_cast<int64_t>(
            skey->unsafe_load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
...
SIGMA_C_FUNC SecretKey_Load(void *thisptr, void *context, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    {
        *in_bytes = util::safe_cast<int64_t>(
            skey->load(*ctx, reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
