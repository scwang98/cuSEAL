...
{
    SIGMAContext::ContextData *cont_data = FromVoid<SIGMAContext::ContextData>(thisptr);
...
SIGMA_C_FUNC ContextData_PrevContextData(void *thisptr, void **prev_data)
{
    ...
    *prev_data = const_cast<SIGMAContext::ContextData *>(cont_data->prev_context_data().get());
    return S_OK;
}
...
SIGMA_C_FUNC ContextData_NextContextData(void *thisptr, void **next_data)
{
    ...
    // The caller should not try to delete the returned pointer
    *next_data = const_cast<SIGMAContext::ContextData *>(cont_data->next_context_data().get());
    return S_OK;
}
