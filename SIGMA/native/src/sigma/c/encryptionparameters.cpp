...
// SIGMANet
#include "sigma/c/encryptionparameters.h"
...
// Enables access to private members of sigma::EncryptionParameters.
using ph = struct sigma::EncryptionParameters::EncryptionParametersPrivateHelper
{
...
SIGMA_C_FUNC EncParams_SetPlainModulus1(void *thisptr, void *plain_modulus)
{
    EncryptionParameters *params = FromVoid<EncryptionParameters>(thisptr);
    IfNullRet(params, E_POINTER);
...
SIGMA_C_FUNC EncParams_Load(void *thisptr, uint8_t *inptr, uint64_t size, int64_t *in_bytes)
{
    ...
    {
        *in_bytes =
            util::safe_cast<int64_t>(params->load(reinterpret_cast<sigma_byte *>(inptr), util::safe_cast<size_t>(size)));
        return S_OK;
    }
    catch (const invalid_argument &)
    {
