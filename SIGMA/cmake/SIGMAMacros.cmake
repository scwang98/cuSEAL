...
macro(sigma_set_language target)
    if(SIGMA_USE_CXX17)
...
macro(sigma_fetch_thirdparty_content content_file)
    set(SIGMA_FETCHCONTENT_BASE_DIR_OLD ${FETCHCONTENT_BASE_DIR})
    set(FETCHCONTENT_BASE_DIR ${SIGMA_THIRDPARTY_DIR} CACHE STRING "" FORCE)
    include(${content_file})
    set(FETCHCONTENT_BASE_DIR ${SIGMA_FETCHCONTENT_BASE_DIR_OLD} CACHE STRING "" FORCE)
    unset(SIGMA_FETCHCONTENT_BASE_DIR_OLD)
endmacro()

# Set the VERSION property
macro(sigma_set_version target)
    set_target_properties(${target} PROPERTIES VERSION ${SIGMA_VERSION})
endmacro()

# Set the library filename to reflect version
macro(sigma_set_version_filename target)
    set_target_properties(${target} PROPERTIES
        OUTPUT_NAME ${target}-${SIGMA_VERSION_MAJOR}.${SIGMA_VERSION_MINOR})
endmacro()

# Set the SOVERSION property
macro(sigma_set_soversion target)
    set_target_properties(${target} PROPERTIES
        SOVERSION ${SIGMA_VERSION_MAJOR}.${SIGMA_VERSION_MINOR})
endmacro()

# Set include directories for build and install interfaces
macro(sigma_set_include_directories target)
...
macro(sigma_link_threads target)
...
macro(sigma_install_target target export)
...
macro(sigma_combine_archives target dependency)
    if(MSVC)
...
macro(sigma_set_secure_compile_options target scope)
    if(MSVC)
