...
namespace SIGMANetTest
{
    ...
    public class SerializationTests
    {
        ...
        {
            Assert.AreEqual(Serialization.SIGMAHeaderSize, 0x10);

            Serialization.SIGMAHeader header = new Serialization.SIGMAHeader();
            Assert.IsTrue(Serialization.IsValidHeader(header));

            Serialization.SIGMAHeader invalidHeader = new Serialization.SIGMAHeader();
            invalidHeader.Magic = 0x1212;
            Assert.IsFalse(Serialization.IsValidHeader(invalidHeader));
            invalidHeader.Magic = Serialization.SIGMAMagic;
            Assert.AreEqual(Serialization.SIGMAHeaderSize, invalidHeader.HeaderSize);
            invalidHeader.VersionMajor = 0x02;
            Assert.IsFalse(Serialization.IsValidHeader(invalidHeader));
            invalidHeader.VersionMajor = SIGMAVersion.Major;
            invalidHeader.ComprMode = (ComprModeType)0x03;
            Assert.IsFalse(Serialization.IsValidHeader(invalidHeader));
        }

        [TestMethod]
        public void SIGMAHeaderSaveLoad()
        {
            Serialization.SIGMAHeader header = new Serialization.SIGMAHeader();
            Serialization.SIGMAHeader loaded = new Serialization.SIGMAHeader();
...
/*
        [TestMethod]
        public void SIGMAHeaderUpgrade()
        {
            LegacyHeaders.SIGMAHeader_3_4 header_3_4 = new LegacyHeaders.SIGMAHeader_3_4();
...
*/
        ...
        {
            {
                SIGMAContext context = GlobalContext.BFVContext;
            ...
            {
                SIGMAContext context = GlobalContext.BGVContext;
                Ciphertext cipher = new Ciphertext();
