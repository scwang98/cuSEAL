...
CMAKE_CXX_COMPILER_CMD=`cmake -LA $SIGMADIR|sed -n 's/^CMAKE_CXX_COMPILER:FILEPATH=\(.*\)$/\1/p'`

echo "Extracting: cmake -LA $SIGMADIR > $CMAKE_ENV"
cmake -LA $SIGMADIR > $CMAKE_ENV
...
FILES=(
	"$SIGMADIR/native/src/sigma/util/config.h"
	"$SIGMADIR/CMakeCache.txt"
	"$SIGMADIR/CMakeFiles/CMakeOutput.log"
	"$SIGMADIR/CMakeFiles/CMakeError.log"
	"/proc/cpuinfo"
	"$CMAKE_ENV"
	"$CMAKE_SYSTEM_INFO"
    "$CMAKE_CXX_COMPILER"
